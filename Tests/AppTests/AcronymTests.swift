...
final class AcronymTests: XCTestCase {
    ...
    func testGettingASingleAcronymFromAPI() throws {
        ...
        let returnedAcronum = try app.getResponse(to: "\(acronymsURI)\(acronym.id!)", decodeTo: Acronym.self)
        
        XCTAssertEqual(returnedAcronum.short, acronymShort)
        XCTAssertEqual(returnedAcronum.long, acronymLong)
        XCTAssertEqual(returnedAcronum.id, acronym.id)
    }
    ...
    func testDeletingAnAcronymFromAPI() throws {
