...
struct WebsiteController: RouteCollection {
    ...
    func acronymHandler(_ req: Request) throws -> Future<View> {
        return try req.parameters.next(Acronym.self).flatMap(to: View.self) { acronym in
            return try acronym.user.get(on: req).flatMap(to: View.self) { user in
                ...
                let context = AcronymContex(title: acronym.short,
                                            acronym: acronym,
                                            user: user,
    ...
    func userHandler(_ req: Request) throws -> Future<View> {
        return try req.parameters.next(User.self).flatMap(to: View.self) { user in
            return try user.acronyms.query(on: req).all().flatMap(to: View.self) { acronyms in
                let context = UserContex(title: user.name, user: user, acronyms: acronyms)
    ...
    func allCategoriesHandler(_ req: Request) throws -> Future<View> {
        ...
        let context = AllCateroriesContext(categories: categories)
    ...
    func editAcronymHandler(_ req: Request) throws -> Future<View> {
        return try req.parameters.next(Acronym.self).flatMap(to: View.self) { acronym in
            let categories = try acronym.categories.query(on: req).all()
            let users = User.query(on: req).all()
            let context = EditAcronymContent(acronym: acronym,
                                             users: users,
                                             categories: categories)
            return try req.view().render("createAcronym", context)
        }
...
struct AcronymContex: Encodable {
    let title: String
    let acronym: Acronym
    let user: User
...
struct AllCateroriesContext: Encodable {
